...
/**
 * Extends the Solr SearchHandler to add in filters based on whether or not a user has been authenticated, and if
 * authenticated, then whether or not the user is an administrator
 *
 * @author waltz
 */
public class LoggingSearchHandler extends SearchHandler implements SolrCoreAware {
     ...
     *
     * DataONE adds authentication information the the request parameters. The parameters are then added to query
     * filters in order to obtain a result set that is appropriate for the level of authorization of the subject
